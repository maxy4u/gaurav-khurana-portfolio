...
export type ServerFetchOptions<TQuery extends QueryWithKey> = Parameters<TQuery>[0] extends
  | Exact<{
      [key: string]: never;
    }>
  | undefined
...
type FetcherReturnValue<TQuery extends QueryWithKey> = Awaited<ReturnType<ReturnType<TQuery['fetcher']>>>;

const cachedServerPreFetch = cache(
  async <TQuery extends QueryWithKey>(useQuery: TQuery, stringifiedQueryOptions?: string) => {
